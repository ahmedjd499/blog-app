...
describe('EditorGuard', () => {
  ...
  it('should deny access for writer user', () => {
    const mockUser = {
      _id: '3',
      username: 'writer',
  ...
  it('should redirect to login if user is not authenticated', () => {
    ...
    const state = { url: '/editor-page' } as any;
    const result = guard.canActivate({} as any, state);
    expect(result).toBe(false);
    expect(router.navigate).toHaveBeenCalledWith(['/login'], { queryParams: { returnUrl: '/editor-page' } });
  });
});
